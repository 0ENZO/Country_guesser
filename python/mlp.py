...
def train_regression_stochastic_backprop_mlp_model(model, inputs, outputs, alpha=0.001, epochs=1000):
    ...
    mylib.train_regression_stochastic_backprop_mlp_model(model,
                                                         final_inputs,
                                                         len(final_inputs),
                                                         final_outputs,
                                                         alpha,
...
def predict_mlp_model_classification(model, sample_inputs, outputs_size=1):
    ...
    return list(np.ctypeslib.as_array(result, (outputs_size,)))


def predict_mlp_model_regression(model, sample_inputs, outputs_size=1):
    sample_inputs = np.array(sample_inputs)
    sample_inputs_type = c_float * len(sample_inputs)
    final_sample_inputs = sample_inputs_type(*sample_inputs)
